...
| Feature / Aspect                  | Python Multiprocessing                                                                                           | Java Virtual Threads (Loom)                                                                    | Kotlin Coroutines (on Dispatchers.Default)                                          |
| --------------------------------- | ---------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| **True CPU-bound parallelism**    | Yes — separate processes, own GIL each                                                                           | Yes — virtual threads on carrier thread pool (≈ cores)                                         | Yes — coroutines dispatched to thread pool (≈ cores)                                |
| **How parallelism works**         | Multiple independent processes (fork/spawn)                                                                      | Lightweight virtual threads mounted on OS carrier threads                                      | Lightweight coroutines scheduled on native thread pool                              |
| **Scalability (number of tasks)** | Limited by OS/RAM (thousands max practical, high overhead per process)                                           | Excellent — millions possible (very low overhead)                                              | Good — thousands to millions (low overhead per coroutine)                           |
| **Overhead**                      | High: process creation (~ms), memory duplication (copy-on-write helps initially), IPC needed (Queues/Pipes slow) | Very low: ~few KB per virtual thread, fast context switch                                      | Very low: ~few KB per coroutine, user-space scheduling                              |
| **Memory sharing**                | Explicit (Queues, shared_memory, Manager) — slow & complex                                                       | Shared heap naturally (all in one JVM process)                                                 | Shared heap (JVM)                                                                   |
| **Startup / Context switch cost** | High (process spawn + serialization)                                                                             | Very low                                                                                       | Lowest (user-level)                                                                 |
| **Best for**                      | CPU-bound batches (data processing, simulations, parallel ML training/inference)                                 | High-concurrency servers (mix I/O + light CPU), legacy sync code                               | Structured async code, flows, cancellation-heavy workflows                          |
| **Drawbacks for CPU-bound**       | High RAM usage (each process loads data/code separately), slow IPC, no easy shared state                         | If task is long CPU-bound without parking, carrier threads can get pinned (but still parallel) | Long CPU-bound suspends block the thread until done (but pool spreads across cores) |
| **Typical speedup**               | Close to linear with cores (e.g., 8-core → ~6–7x vs single)                                                      | Good, but virtual threads shine more on I/O; CPU similar to platform threads                   | Good on multi-core, but tuned thread pool limits max parallelism to cores           |
