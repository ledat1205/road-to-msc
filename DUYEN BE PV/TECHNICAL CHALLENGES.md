- Slow code due to wrong usage of framework vertx (not offloading heavy task) + not using redis for idempotency checking but relying on db + not releasing lock (for idempotency) while hanlding error requests
