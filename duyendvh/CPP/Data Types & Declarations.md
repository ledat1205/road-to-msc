...
| **volatile**          | Tells the compiler that a variable's value may be changed by external factors (e.g., hardware), preventing aggressive optimization.        | Used in multi-threaded or embedded programming.<br>volatile int* status = reinterpret_cast<int*>(0x40000000);<br><br>while ((*status & 1) == 0) {<br>    // compiler must reload value every iteration<br>}<br>                             |
