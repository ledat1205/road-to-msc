...
| **using**                                                 | ## Basic example<br><br>`using score = int; score s = 100;`<br><br>Same result, **much better syntax**.<br><br>---<br><br>## Why `using` is superior<br><br>### 1️⃣ Much clearer with complex types<br><br>`using Func = int(*)(int, int);`<br><br>Readable left-to-right.<br><br>---<br><br>### 2️⃣ Works with templates (major win)<br><br>`template<typename T> using Vec = std::vector<T>;  Vec<int> v;  // std::vector<int>`<br><br>This alone is enough reason to prefer `using`.<br><br>---<br><br>### 3️⃣ Enables cleaner generic code<br><br>`template<typename T> struct Traits {     using value_type = T; };`<br><br>Used **everywhere** in STL and modern libraries.<br><br>---<br><br># `using` for namespaces<br><br>### Old way<br><br>`std::cout << "Hello"; std::vector<int> v;`<br><br>### Using declaration<br><br>`using std::cout; using std::vector;  cout << "Hello"; vector<int> v;`<br><br>---<br><br>### Using directive (be careful)<br><br>`using namespace std; // ❌ in headers`<br><br>Rule of thumb:<br><br>- ❌ Avoid in headers<br>    <br>- ⚠ Acceptable in `.cpp` or very small scopes | `using T = int;`                                                                                                                                                                                             |
