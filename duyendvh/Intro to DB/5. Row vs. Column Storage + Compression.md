...
### Parquet — The #1 Data Lake Format

### **1️⃣ Columnar Storage**

- Stores data **by column**, not by row.
    
- Scans only needed columns → **10×–100× less I/O**.
    

### **2️⃣ High Compression**

- Per-column compression: RLE, dictionary, delta, bit-packing, Snappy, ZSTD.
    
- Similar values → great compression → **CSV 1GB → Parquet ~120MB**.
...
- Engines skip entire row groups → **huge speedups**.
    

### **4️⃣ Self-Describing Schema**
...
### **5️⃣ Vectorized Execution**

- Columnar blocks map to SIMD → **CPU processes thousands of values at once**.
    

### **6️⃣ Row Groups & Pages**
...
- Can be split at row group boundaries → perfect for distributed systems.
    

### **8️⃣ Immutable-Friendly**

- Write-once, append-only → ideal for S3/GCS/HDFS data lakes.
    

### **9️⃣ Widest Ecosystem Support**
...
|**Tuple-level**|Entire row|PostgreSQL, SQL Server|Good for OLTP|Can’t use cross-row redundancy|
...
- “users who visited page A AND page B”
    
- “events for user_id IN (…)”
    
- “how many unique users?”
    
- “filter rows where country_id ∈ {3,7,9}”
...
`Container 0 → bits for [5,7,1000] Container 1 → bits for [4]`

---

# ⭐ Roaring Compresses Each Container

For each container:

📌 If dense (many bits 1) → **bitmap container**  
📌 If sparse (few bits 1) → **array container**  
📌 If small ranges → **run container**
...
- container 0 → [1,2,3,4] → run-length encoded
    
- container 15 → [16960] (1000000 >> 16 = 15)
    

Compressed size might be 20–40 bytes.

---

# 🔥 Example: Bitmap Encoding in ClickHouse
...
# 🧠 Why Bitmap Encoding Is Great (Especially in ClickHouse)

### ✔ Fast Set Operations (SIMD-accelerated)

Bitmap AND/OR/XOR → bitwise ops on CPU → **billions of values per second**

### ✔ Efficient for repeated filtering

Useful for “precomputed segments,” e.g., users who:
...
Roaring bitmaps compresses sparse data 10×–100×.

---

# 🧪 Real Example: How ClickHouse Stores Bitmaps
...
![[Screenshot 2025-12-11 at 00.16.52.png]]