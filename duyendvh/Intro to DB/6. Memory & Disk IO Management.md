...
![[Screenshot 2025-12-11 at 19.07.11.png]]
![[Screenshot 2025-12-11 at 19.09.13.png]]

![[Screenshot 2025-12-11 at 19.10.42.png]]
![[Screenshot 2025-12-11 at 18.56.14.png]]
# ⭐ What is Memory-Mapped I/O?

Memory-mapped I/O means:

`OS maps a file from disk → directly into process memory. The DBMS reads/writes by touching memory addresses. The OS handles actual I/O behind the scenes.`
...
But for real databases → **DISASTER**.
...
> OS cannot guarantee crash safety → only DBMS can.

### Real-world:

SQLite uses mmap optionally → but **only for reads**, never for writes.

---

# 🚨 Problem #2 — I/O Stalls

### ❌ “DBMS doesn’t know which pages are in RAM.”

With mmap:

- DB accesses the memory address
    
- If the page is not in RAM → Page fault
    
- OS loads page from disk
    
- Thread is blocked until disk read finishes → **I/O stall**
...
With mmap → all of that is impossible.

### Example:

Clicking row in a 20GB table leads to page fault → disk read → thread freeze.

For OLTP systems, stalls = death.

---

# 🚨 Problem #3 — Error Handling (SIGBUS)

### ❌ “Any memory access can trigger SIGBUS.”
...
mmap maps file offsets → memory addresses.

If DB tries to read:

`address = 0xABCDEF`

But file no longer has that offset → SIGBUS.
...
# 🚨 Problem #4 — Performance Issues (TLB Shootdowns)
...
`→ Kernel must invalidate TLB entries across all CPU cores → Expensive cross-core IPI (inter-processor interrupts) → Performance collapse`
...
Databases like Postgres commonly run on machines with **64–256 CPU cores**, so TLB shootdowns are a huge bottleneck.
