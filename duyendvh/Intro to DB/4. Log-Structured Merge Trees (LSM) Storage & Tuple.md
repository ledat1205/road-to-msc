...
## 🥇 Step 1 — Query the Memtable (RAM)

🔥 Always checked first because:

- It has **newest writes**
    
- It’s in **RAM** → fastest access
...
## Step 2 — Immutable Memtables (Not Yet Flushed)
...
2. **If Bloom filter might match → binary-search the Index Block**
...
👉 There is **exactly one SST file at L1** that _can possibly_ contain this key.
...
(Sometimes 0 files if key’s range doesn’t exist.)
...
NULL bitmap (4 columns → 1 byte enough):
...
# 🟦 **How PostgreSQL Compresses Columns (TOAST compression)**

## 1️⃣ **When does compression happen?**

PostgreSQL **automatically** tries to compress a column when:

- The row does **not** fit inside an 8 KB page, **AND**
    
- The column’s storage mode allows compression  
    (default for most variable-length types)
    

Compression order:

1. 🔹 Compress large varlena values
    
2. 🔹 If still too large → move to TOAST table
    
3. 🔹 If still too large → split into TOAST chunks
...
# 2️⃣ **What compression algorithm is used?**

PostgreSQL uses:

### ✔ **pglz (PostgreSQL LZ variant)**

- A custom LZ77-based algorithm
    
- Fast, low-memory, optimized for short strings
    
- Typically 30–50% compression ratio
...
# 3️⃣ **What data types are compressed?**
...
# 4️⃣ **How compression actually works inside a tuple**
...
### Step B — Try pglz or LZ4 compression
...
- **Must reduce size** by at least **25%**
    ...
    - If not → “not worth it” → store uncompressed
        
    - Prevents wasting CPU on poorly compressible values
        

### Step C — Store compressed varlena
...
# 6️⃣ **Example: 100 KB JSONB INSERT**
...
# 7️⃣ **Why compress BEFORE TOASTing?**

Because:

### ✔ Fewer TOAST chunks

(Chunk count ↓ = faster reads/writes)
...
# 8️⃣ **Can you disable compression?**

Yes. Each column can define storage mode:

- `PLAIN` → no compression, no TOAST
    
- `EXTENDED` → compression + TOAST
    
- `EXTERNAL` → no compression, but can move to TOAST
    
- `MAIN` → attempt to keep inline, avoid TOAST
...
If very large → stored outside (TOAST), and tuple stores only a pointer.
...
2. If still too large → move to TOAST
...
✔ PostgreSQL supports values up to **1 GB per field**  
✔ Page size does NOT limit row size  
✔ Only small metadata stays in main page

---

### 🟡 MySQL (InnoDB) – Overflow Pages & Off-page Storage
...
- First 768 bytes → inline in the clustered index page
    
- Remaining 99 KB → stored in overflow pages
...
## 1. `SELECT pg_column_size(ROW(1,2,3));` → 36 bytes
...
- 3 fields → 12 bytes of actual data
...
`23 (header) + 1 padding = 24 → aligned to 4 bytes   24 + 12 (data) = 36`

---

## 2. `SELECT pg_column_size(ROW(1::int2,2::int2,3::int2));` → 30 bytes
...
23 is odd → 1 byte padding to align to 2 bytes.

Total:

`23 + 1 + 6 = 30 bytes`

✔ No alignment required between fields because `int2` only needs 2-byte alignment.

---

## 3. `SELECT pg_column_size(ROW('a'::char, 2::int2, 'b'::char, 4::int4, 'c'::char, 8::int8));` → 48 bytes
...
✔ Most bloat comes from aligning before int4 and int8.

---

## 4. `SELECT pg_column_size(ROW(8::int8, 4::int4, 2::int2, 'a'::char, 'b'::char, 'c'::char));` → 44 bytes
...
# 🟦 1. Two Big Categories of Numeric Types

## ✔ **1. Fixed-precision (binary fixed-size)**
...
## ✔ **2. Variable-precision (arbitrary precision / decimal)**
...
- Much slower (10x–200x slower than integer arithmetic)
...
# 🟦 2. Why CPU cannot handle DECIMAL natively
...
👉 That’s why DECIMAL/NUMERIC is slow.

---

# 🟦 3. How databases store DECIMAL/NUMERIC internally

## ✔ PostgreSQL `NUMERIC(p,s)` internal format
...
- Each element in `digits[]` is **0–9999**
    
- 4 decimal digits per “digit”
    
- It grows or shrinks depending on the number’s precision → **variable length**
...
## ✔ MySQL `DECIMAL(p,s)` internal storage
...
Example: `DECIMAL(15,5)` → max value = `999999999.99999`

Stored as:

`[000999999][99999_____]    4 bytes     3 bytes`

Using fixed-size groups — so the total storage is:

`INT(9)*4 bytes + fractional groups`

---

## ✔ Oracle `NUMBER(p,s)` internal storage
...
# 🟦 4. How CPU performs arithmetic on DECIMAL
...
- `NUMERIC` addition: **50–500 cycles**
    
- `NUMERIC` multiplication: **200–2000 cycles**
...
# 🟦 5. Why DBs support both types

|Type|Speed|Precision|Use case|
|---|---|---|---|
|INT / BIGINT|⭐⭐⭐⭐⭐|Exact|IDs, counters|
|FLOAT / DOUBLE|⭐⭐⭐⭐|approx|scientific, analytics|
|DECIMAL / NUMERIC|⭐|exact decimal|money, finance|
...
`amount + 10.00 → convert 10.00 to same scale → align weight → add digit arrays → recalc carry`
...
- integer part = 6 digits → 4 bytes
    
- fractional part = 4 digits → 4 bytes
...
# 🟦 7. Why fixed-precision types are faster
