...
`+-------------------------+ | Page Header             | +-------------------------+ | Item/Line Pointers      | +-------------------------+ | Tuples (physical data)  | +-------------------------+ | Free Space              | +-------------------------+`
...
`+------------------------------+ | Tuple Header                 | +------------------------------+ | NULL Bitmap (optional)       | +------------------------------+ | Variable-length data         | +------------------------------+ | Fixed-length data            | +------------------------------+ | Padding (alignment)          | +------------------------------+`
...
`TupleHeader {     xmin      // creating transaction ID     xmax      // deleting or updating txn ID     xvac      // vacuum ID (old)     ctid      // tuple identifier (version chaining)     infomask  // flags: nulls? varlen? keys?     natts     // number of attributes     ... other MVCC/control bits }`
...
`[Header]     xmin = 100     xmax = 0     ctid = (0,10)     null_bitmap = 0b000  (no nulls)  [Varlen Area]     "Alice"  [Fixed Area]     id = 10     age = 25`

---

# 7. What Happens on UPDATE?

UPDATE does **not** overwrite the tuple.  
It creates a **new tuple**, chains via `ctid`.

Example:

`Old tuple (version 1):     xmin=100, xmax=120, ctid -> self  New tuple (version 2):     xmin=120, xmax=0, ctid -> self`

Readers with snapshot SCN < 120 → see old version  
Readers with snapshot ≥ 120 → see new version

This is how **MVCC** works internally.
...
The tuple remains until VACUUM or garbage collection.

---

# 9. Variable-length Attributes (Problem)

For varlen types (`TEXT`, `VARCHAR`, `BYTEA`):

- They require special flags in header.
    
- They may be **TOASTed** (moved out-of-page) if too large.
    
- The tuple only stores a pointer to the external storage.
    

---

# 10. Why Tracking Tuple Count Is Not Enough

If you just “append” tuples:

❌ **Deleting tuples** causes holes  
❌ **Updating tuples** creates chains  
❌ **Variable-length attributes** cannot be overwritten safely
...
- tuple headers with MVCC metadata
    
- version chains
    
- UNDO (Oracle) or UNDO-like MVCC (PostgreSQL)
