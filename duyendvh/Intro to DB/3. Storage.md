https://15445.courses.cs.cmu.edu/fall2024/slides/03-storage1.pdf

![[Screenshot 2025-12-07 at 17.19.48.png]]


![[Screenshot 2025-12-07 at 17.39.12.png]]
![[Screenshot 2025-12-07 at 17.45.47.png]]
![[Screenshot 2025-12-07 at 17.46.13.png]]

![[Screenshot 2025-12-07 at 17.41.54.png]]


![[Screenshot 2025-12-07 at 18.19.47.png]]

![[Screenshot 2025-12-07 at 18.17.55.png]]
# 🟩 **1. ORACLE BLOCK (SELF-CONTAINED)**

### Example: A single Oracle table block with 3 rows
![[Screenshot 2025-12-07 at 18.28.22.png]]

`+-------------------------------------------------------------+ |                       BLOCK HEADER                          | |  - Block Type = Data                                        | |  - Object ID = 1257 (table X)                               | |  - SCN = 8123919                                             | |  - ITL Entries:                                             | |       [0] TX#45, status ACTIVE                               | |       [1] TX#39, status COMMITTED                            | |  - Checksum                                                 | +-------------------------------------------------------------+ |                     TABLE DIRECTORY                          | |  Table#0   Offset 200                                        | +-------------------------------------------------------------+ |                     ROW DIRECTORY (Slots)                   | |   Slot 0 → offset 350 (Row ID: 5)                            | |   Slot 1 → offset 420 (Row ID: 6)                            | |   Slot 2 → offset 510 (Row ID: 7)                            | +-------------------------------------------------------------+ |                         FREE SPACE                          | |            (grows/shrinks as rows inserted/deleted)         | +-------------------------------------------------------------+ |                       ROW DATA                              | |   Row @350:   ITL=0  Length=25 bytes  Undo=UB 10:3          | |   Row @420:   ITL=1  Length=32 bytes  Undo=UB 9:7           | |   Row @510:   ITL=1  Length=40 bytes  Undo=UB 9:8           | +-------------------------------------------------------------+`

### ⭐ What you see here:

- **Everything needed** (locks, visibility, undo pointer, SCN, row map) is in the block
    
- Oracle can interpret every byte **without consulting external structures**
    

---

# 🟥 **2. POSTGRESQL PAGE (NOT SELF-CONTAINED)**

### Example: PostgreSQL heap page with 3 tuples

`+-------------------------------------------------------------+ |                         PAGE HEADER                         | |  - LSN                                                      | |  - Page flags                                               | |  - Free space pointer                                       | |  - Lower/Upper pointers                                     | +-------------------------------------------------------------+ |                     LINE POINTER ARRAY                      | |   Item 1 → offset 300                                       | |   Item 2 → offset 350                                       | |   Item 3 → offset 500                                       | +-------------------------------------------------------------+ |                         FREE SPACE                          | +-------------------------------------------------------------+ |                     TUPLE DATA (3 rows)                     | |   Tuple 1 @300: xmin=140, xmax=0, ctid=(5,1), columns...    | |   Tuple 2 @350: xmin=80, xmax=141, columns...               | |   Tuple 3 @500: xmin=141, xmax=0, columns...                | +-------------------------------------------------------------+`

### 🚨 Important:

PostgreSQL page **does NOT include**:

- commit status (stored in CLOG)
    
- lock info (external lock manager)
    
- snapshot visibility info
    
- undo pointer (Postgres does NOT use undo in blocks)
    

### Result:

To check whether Tuple 1 is visible:

1. Look at tuple → `xmin`
    
2. Check PostgreSQL **commit log** → was `xmin` committed?
    
3. Compare with snapshot → visible or not?
    
4. If dead → VACUUM must clean it later
    

This is why **PostgreSQL pages are NOT self contained**.

---

# 🟦 **3. MYSQL INNODB PAGE (PARTIALLY SELF-CONTAINED)**

### Example: InnoDB data page (16KB)

`+-------------------------------------------------------------+ |                       FILE PAGE HEADER                      | |  - Checksum                                                 | |  - LSN                                                      | |  - Page type (data/index/undo)                              | |  - Prev/Next page pointers                                  | +-------------------------------------------------------------+ |                      PAGE DIRECTORY                         | +-------------------------------------------------------------+ |                       RECORD DIRECTORY                      | |   Slot 0 → offset 1024                                      | |   Slot 1 → offset 1100                                      | |   Slot 2 → offset 1200                                      | +-------------------------------------------------------------+ |                         FREE SPACE                          | +-------------------------------------------------------------+ |                       ROW RECORDS                           | |   Row @1024: trx_id=272, roll_ptr=UB 3:21, data...          | |   Row @1100: trx_id=272, roll_ptr=UB 3:22, data...          | |   Row @1200: trx_id=271, roll_ptr=UB 3:10, data...          | +-------------------------------------------------------------+`

### ⭐ Important:

- Has undo pointer → allows reconstructing old versions
    
- Stores transaction ID → can detect latest change
    
- BUT locking is stored **outside** the page
    

This makes it _semi self-contained_.

---

# 🟨 **4. VISUAL COMPARISON (EASY TO REMEMBER)**

`Oracle Block (self-contained)  ├─ header (object+SCN+ITL)  ├─ row directory  ├─ rows (with undo pointers)  └─ everything is inside ★  Postgres Page (not self-contained)  ├─ header  ├─ line pointers  ├─ rows (xmin/xmax)  └─ needs external commit log, locks, VACUUM ✖  InnoDB Page (half self-contained)  ├─ header  ├─ directory  ├─ rows (trx_id + undo)  └─ external lock manager and dict lookups △`

---

# 🧠 Why Oracle looks different:

### Oracle page = **a mini universe**

Everything needed for:

- visibility
    
- consistency
    
- recovery
    
- locking
    

is inside the block.  
That's why Oracle pages are called **self-contained**.

### PostgreSQL page = **just tuples**

Needs:

- CLOG
    
- Visibility map
    
- Lock manager
    
- VACUUM
    
- Snapshot checks
    

Bad for OLTP concurrency.

### InnoDB page = **mini universe but no locking**

Undo inside, but locking external.

---
# 🧠 **1. How PostgreSQL Organizes Data Internally**

PostgreSQL stores its data using **a layered structure**:

`Table → Pages (Blocks) → Tuples (Rows)`

### ✅ **A. Table is stored as many 8 KB pages**

- Every table (and index) on disk is broken into fixed-size blocks.
    
- By default: **1 page = 8 KB**
    

Example:  
If you have a 1 GB table → it contains ~131,000 pages.

### Why pages?

- Easy to cache.
    
- Easy to load from disk.
    
- Easy to manage during vacuum, MVCC, WAL.
    

---

### ✅ **B. Shared Buffers = Internal PostgreSQL Cache (in RAM)**

PostgreSQL does **not** read rows directly into memory.

Instead:

1. PostgreSQL requests a **page** (8 KB)
    
2. Page is loaded into **shared_buffers**
    
3. All future queries use this page from memory—**no disk**.
    

---

### 🧠 **C. Shared Buffers acts like PostgreSQL’s “brain memory”**

- It’s a **fixed-size RAM area** configured by:
    
    `SHOW shared_buffers;`
    
    Example: `4GB`
    
- It stores recently used pages of:
    
    - Tables
        
    - Indexes
        
    - Toast
        
    - Heap pages
        
    - Visibility maps
        

---

# 📌 **2. Why PostgreSQL reads MOST data from memory**

Postgres uses **two layers of caching**:

                     `┌──────────────────────┐                      │ OS Page Cache (RAM)  │                      └──────────────────────┘                              ▲                              │                      ┌──────────────────────┐                      │ Postgres shared_buffers │                      └──────────────────────┘                              ▲                              │                      ┌──────────────────────┐                      │ Disk (SSD/HDD)       │                      └──────────────────────┘`

## ✔ Layer 1: PostgreSQL Cache (shared_buffers)

- Holds frequently accessed pages.
    
- Most queries are served from here.
    
- Reduces disk reads dramatically.
    

## ✔ Layer 2: OS Page Cache

If a page is not in shared_buffers:

- PostgreSQL asks OS to read the page.
    
- OS loads it into **page cache** (RAM).
    
- So even disk reads are actually mem reads if OS cached it.
    

---

# 🎯 **Result: even when PostgreSQL reads "from disk", it may still be from RAM**

This is why PostgreSQL can often achieve > 99% cache hit ratio.

Check it:

`SELECT   sum(blks_hit) * 100.0 /   nullif(sum(blks_hit + blks_read), 0) AS cache_hit_ratio FROM pg_stat_database;`

---

# 🚀 3. **What Exactly Is a Buffer?**

A **buffer** = one 8 KB page of a table/index loaded into memory.

A **buffer** contains:

- Header (page metadata)
    
- Rows (tuples)
    
- Free space
    
- Visibility info (MVCC)
    

When you run a SELECT:

1. PostgreSQL checks if the page is in shared buffers.
    
2. If YES → **memory hit** (very fast)
    
3. If NO → read page from disk → place it into a buffer.
    

---

# 🧠 4. **Why PostgreSQL uses fixed-size (8 KB) pages**

Advantages:

### ✔ Predictability (performance)

- Always reads/write full 8 KB chunks.
    
- Very efficient with SSD block sizes.
    

### ✔ Easy caching

- LRU-like behavior for frequently accessed pages.
    
- Predictable memory usage.
    

### ✔ MVCC works better

- Row versions stored inside pages.
    
- Vacuum can track free space page-by-page.
    

---

# 🔥 5. Example: How a SELECT works under the hood

Let's say you run:

`SELECT * FROM users WHERE id = 10;`

### Step-by-step:

1. Planner decides it needs index `users_pkey`
    
2. PostgreSQL loads the index page(s) into buffers
    
3. Using index → finds heap page number
    
4. PostgreSQL loads that heap page into buffers
    
5. Row is returned → **all future reads now served from memory**
    

This is why PostgreSQL becomes extremely fast **after warm-up**.

---

# 🧠 6. How PostgreSQL Keeps Hot Data in Memory

Shared buffers grows hotter by:

### ✔ Frequently accessed pages stay longer

Because they stay in LRU end of buffer chain.

### ✔ Rarely accessed pages get evicted

When shared_buffers is full, the least recently used pages are removed.

---

# 🧪 7. How to see what is in memory (buffers)

`SELECT * FROM pg_buffercache LIMIT 20;`

(Needs extension `pg_buffercache`)

You can see:

- Which tables are cached
    
- Which indexes are cached
    
- How many pages are cached per relation
# 
🚀 1. **Storage & Table Size Monitoring**

## **📌 pg_database_size(name)**

**Concept:**  
Shows how much space a database consumes on disk.

**Why it matters:**  
Use it to detect which database is growing too fast or which one is too big.

**Example**

`SELECT pg_size_pretty(pg_database_size('mydb'));`

→ Returns human-readable size like `12 GB`.

---

## **📌 pg_relation_size(object)**

**Concept:**  
Returns **size of a single table OR index only** (WITHOUT children like TOAST or indexes).

**Useful when:**  
You want to know if a table or index is getting too big.

**Example**

`SELECT pg_size_pretty(pg_relation_size('orders'));`

---

## **📌 pg_total_relation_size(object)**

**Concept:**  
Returns **table + indexes + toast + toast indexes**.  
This is the **real total size** of a table.

**Example**

`SELECT pg_size_pretty(pg_total_relation_size('orders'));`

→ Use this to see how large a table REALLY is.

---

## **📌 pg_indexes_size(table)**

**Concept:**  
Shows total storage used **by all indexes** on a table.

**Why important:**  
Indexes can grow silently and consume more space than the table itself.

**Example**

`SELECT pg_size_pretty(pg_indexes_size('orders'));`

---

## **📌 pg_table_size(table)**

**Concept:**  
Shows size of table data (**excluding indexes**)  
but **including TOAST** (large text/binary).

**Example**

`SELECT pg_size_pretty(pg_table_size('orders'));`

---

## **📌 pg_tablespace_size(name)**

**Concept:**  
A tablespace is a directory on disk where data is stored.  
This function shows how much data is inside it.

**Example**

`SELECT pg_size_pretty(pg_tablespace_size('pg_default'));`

---

# 🚀 2. **Cache & Buffer Monitoring**

## **📌 Cache Hit Ratio**

**Concept:**  
Measures how often PostgreSQL reads data **from memory (fast)**  
instead of **disk (slow)**.

High-performance DBs have **> 99% hit ratio**.

**Example**

`SELECT   sum(blks_hit) / nullif(sum(blks_hit + blks_read), 0) AS cache_hit_ratio FROM pg_stat_database;`

---

## **📌 pg_statio_user_tables**

**Concept:**  
Shows how many blocks were read from:

- **disk** (`heap_blks_read`)
    
- **cache** (`heap_blks_hit`)
    

**Use case:**  
Find tables that cause disk I/O.

**Example**

`SELECT relname, heap_blks_read, heap_blks_hit FROM pg_statio_user_tables;`

---

# 🚀 3. **Index Monitoring**

## **📌 pg_stat_user_indexes**

**Concept:**  
Tracks how often an index was used (`idx_scan`).

**Why:**  
Unused indexes slow down writes and waste disk.

**Example — find unused indexes**

`SELECT     indexrelname, idx_scan FROM pg_stat_user_indexes WHERE idx_scan = 0;`

---

## **📌 High Sequential Scans = Missing index**

**Concept:**  
If a table is scanned sequentially (`seq_scan`) more than using indexes (`idx_scan`),  
it suggests indexes are missing or are not selective.

**Example**

`SELECT relname, seq_scan, idx_scan FROM pg_stat_user_tables WHERE seq_scan > idx_scan;`

---

# 🚀 4. **Vacuum / Autovacuum Monitoring**

## **📌 n_dead_tup (dead tuples)**

**Concept:**  
Dead rows created by UPDATE/DELETE.  
PostgreSQL MUST vacuum them to free space.

**Example**

`SELECT relname, n_dead_tup FROM pg_stat_user_tables ORDER BY n_dead_tup DESC;`

---

## **📌 last_vacuum / last_autovacuum**

**Concept:**  
Shows when PostgreSQL last cleaned the table.

**Example**

`SELECT relname, last_vacuum, last_autovacuum FROM pg_stat_user_tables;`

---

## **📌 pgstattuple (needs extension)**

**Concept:**  
Estimates table **bloat** — wasted space due to dead tuples.

**Example**

`SELECT * FROM pgstattuple('orders');`

---

# 🚀 5. **WAL / Checkpoint Monitoring**

## **📌 pg_stat_bgwriter**

**Concept:**  
Shows how often PostgreSQL performs checkpoints and buffer writes.  
Used to detect I/O pressure.

**Example**

`SELECT * FROM pg_stat_bgwriter;`

---

# 🚀 6. **Lock Monitoring**

## **📌 pg_stat_activity (wait events)**

**Concept:**  
Shows what each backend is doing and if it is **waiting** on a lock.

**Example**

`SELECT     pid, usename, wait_event_type, wait_event, query FROM pg_stat_activity WHERE wait_event_type IS NOT NULL;`

---

## **📌 pg_locks**

**Concept:**  
Shows all locks currently held (row locks, table locks, etc.)

**Example**

`SELECT * FROM pg_locks;`

---

# 🚀 7. **Connection Monitoring**

## **📌 Number of connections**

**Concept:**  
Tracks all active connections to the DB.

**Example**

`SELECT datname, count(*) FROM pg_stat_activity GROUP BY datname;`

---

# 🚀 8. **Slow Query Monitoring**

## **📌 pg_stat_statements**

**Concept:**  
Tracks execution time, frequency, and total cost of each query.

**Why:**  
Most important performance extension.

**Example — slowest queries**

`SELECT query, mean_exec_time, calls FROM pg_stat_statements ORDER BY mean_exec_time DESC LIMIT 20;`

---

# 🚀 9. **Table Statistics**

**Concept:**  
Shows table insert/update/delete activity and dead/live tuples.

**Example**

`SELECT     relname,     n_tup_ins, n_tup_upd, n_tup_del,     n_live_tup, n_dead_tup FROM pg_stat_user_tables;`

---

# 🚀 10. **Freeze / XID Monitoring (pg_relation_*)**

## **📌 pg_relation_frozen_xid(oid)**

**Concept:**  
PostgreSQL uses transaction IDs (XIDs).  
If old rows aren't vacuum-frozen in time, **transaction wraparound** can crash the DB.

This function shows the oldest unfrozen transaction per table.

**Example**

`SELECT relname, pg_relation_frozen_xid(oid) FROM pg_class WHERE relkind='r';`

---

# 🚀 11. **Free Space Monitoring**

## **📌 pg_freespace(rel)**

Requires: `pg_freespacemap`

**Concept:**  
Shows how much free space is available in each page of a table.

**Use case:**  
Detect tables with fragmentation.

**Example**

`SELECT * FROM pg_freespace('orders') LIMIT 10;`

---