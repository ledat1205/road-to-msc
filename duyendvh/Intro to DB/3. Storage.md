...
Checksum:                0xA37F
LSN (Log Sequence #):    42,981,551
DBMS Version:            PostgreSQL 16
Page Type:               Heap Data Page

Transaction Visibility:
    xmin:                10517   (transaction that inserted row)
    xmax:                0       (still alive)
    flags:               HOT chain, visible to everyone

Free Space Pointer:      2450 bytes used, 5742 free
PD Lower:                offset 240
PD Upper:                offset 7950

Compression / Encoding:
    TOAST compressed:    yes
    Dictionary ID:       12
    Encoding:            UTF-8
...
Schema Metadata:
    Table OID:           31125
    Attribute Count:     5
    Column Types:        int4, text, text, timestamptz, float8

Sketches / Summary Stats:
    Min Key:             1002
    Max Key:             1075
    Bloom Filter:        hash(buyer_id)
    Null Count (approx): 8
    Distinct Estimate:   57

![[Screenshot 2025-12-07 at 18.19.47.png]]

![[Screenshot 2025-12-07 at 18.17.55.png]]
# 🟥 What is the **block header** (“head file”)?

Each Oracle block begins with a **block header** that stores metadata about the block.

Here is the structure:

`+-----------------------------------+ |           Block Header            | | (file#, block#, SCN, type, flags) | +-----------------------------------+ |      Table/Row Directory          | +-----------------------------------+ |            Free Space             | +-----------------------------------+ |           Row Data Area          | +-----------------------------------+`

---

# 🟨 **The Block Header Contains:**

### **1. Block Addressing Information**
...
- Block type (data, index, undo, segment header, etc.)
    

### **2. System Change Number (SCN)**

- Identify when block was last modified
    
- Allows consistent reads (Oracle’s version of MVCC)
    

### **3. Transaction Information (ITL – Interested Transaction List)**

This stores:

- which transactions are modifying rows inside this block
...
This is _unique to Oracle_ and one reason each block must be self-contained.

### **4. Row Directory**

Instead of storing rows contiguously:

`[ Row 1 ] [ Row 3 ] [ free ] [ Row 2 ]`

Oracle stores offsets to rows in the header:

`slot 0 → offset to Row 1   slot 1 → offset to Row 3   slot 2 → offset to Row 2`  
...
- compaction without breaking pointers
    

### **5. Free Space Pointer**

Tracks free space inside the block.
