...
check_running() {
    # Check if the PID file exists and the process ID is active
...
case "$1" in
    start)
        ...
        # --exec points to the executable, --pidfile specifies where to write the PID
    ...
    restart)
        $0 stop # Call the stop action
        sleep 1
        $0 start # Call the start action
        ;;
